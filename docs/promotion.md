...
> Free LLM API aggregator powered by Cloudflare Workers. No local service or VPS required, with custom domains and OpenAI / Anthropic compatible APIs.
...
最近把自己日常使用的几个免费 LLM API 整合成了一个 Cloudflare Worker：**LLMflare**。

它主要解决一个很实际的问题：Gemini、Groq、OpenRouter、NVIDIA 和 Workers AI 等平台都有各自的免费模型或免费额度，但地址、密钥和模型名很分散。现在可以用一个 Base URL 管理多个 Provider，并把它们统一成常见的接口：
...
项目不需要 VPS，也不占用本机资源，直接部署到 Cloudflare Workers；可以使用 Workers 地址，也可以绑定自己的域名。在许多国内网络环境中，客户端只需连接这个统一入口，不必单独为每个国外 API 配置代理。后台可以编辑 Provider 和 API Key、自动获取模型、设置模型白名单、生成客户端访问密钥，并查看 Token、TPS 和首字延迟等用量数据。

隐私方面，不保存 Prompt 和模型回复，只记录 Provider、模型、Token、耗时、状态码等统计元数据。Provider Key 保存在部署者自己的 Cloudflare KV 中。

需要说明的是：无服务器不代表绝对不会中断，免费模型也不代表无限额度，网络可用性仍受 Cloudflare、上游平台和地区线路影响。项目也能添加收费 API，但更建议只放免费服务或严格限额的专用 Key，避免密钥泄露后产生费用损失。
...
我平时会同时用 Gemini、Groq、OpenRouter、NVIDIA 和 Workers AI，但很多客户端需要重复维护 Base URL、Key 和模型列表，所以做了一个自用为主的开源网关 LLMflare。
...
项目地址：<https://github.com/Jiao77/LLMflare>
...
I built **LLMflare**, a free and self-hosted LLM API aggregator for people who use several providers and do not want to maintain a separate base URL and key in every client.
